...
typedef long long_t;  //int_64 -> int
typedef unsigned char cc_t;
typedef enum { FALSE, TRUE } bool_t;  //bool

/* Y64 Condition Code */  //zso
#define GET_ZF(cc) (((cc) >> 2)&0x1)
#define GET_SF(cc) (((cc) >> 1)&0x1)
#define GET_OF(cc) (((cc) >> 0)&0x1)

#define PACK_CC(z,s,o) (((z)<<2)|((s)<<1)|((o)<<0))  //cc = zso

#define DEFAULT_CC PACK_CC(1,0,0)  //default_cc
...
typedef enum { REG_ERR=-1, REG_RAX, REG_RCX, REG_RDX, REG_RBX,
    ...
    REG_R11, REG_R12, REG_R13, REG_R14, REG_NONE } regid_t;  //register

#define NORM_REG(_id) ((_id) >= REG_RAX && (_id) <= REG_R14)  //normal register
#define NONE_REG(_id) ((_id) == REG_NONE)  //none_register
...
typedef enum { I_HALT = 0, I_NOP, I_RRMOVQ, I_IRMOVQ, I_RMMOVQ, I_MRMOVQ,
    I_ALU, I_JMP, I_CALL, I_RET, I_PUSHQ, I_POPQ, I_DIRECTIVE } itype_t;  //instruction

/* Function code (default) */
typedef enum { F_NONE } func_t;   

/* ALU code */
typedef enum { A_ADD, A_SUB, A_AND, A_XOR, A_NONE } alu_t;  //ALU

/* Condition code */
typedef enum { C_YES, C_LE, C_L, C_E, C_NE, C_GE, C_G } cond_t;  //condition
