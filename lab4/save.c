...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
		...
		{
			err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, tmp);
			return STAT_ADR;
		}
      ...
      case I_CALL: /* 8:x imm */
		if(cond_doit(sim->cc, ifun))
		{
			tmp = get_reg_val(sim->r, REG_RSP); //get %esp
			tmp -= 8;
			set_reg_val(sim->r, REG_RSP, tmp); //set %esp
			if(!set_long_val(sim->m, tmp, next_pc)) //set [%esp]
			{
				err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, tmp);
				return STAT_ADR;
			}
			sim->pc = imm;
		}
      ...
      case I_PUSHQ: /* A:0 regA:F */
		...
		tmp = get_reg_val(sim->r, regA); //get %regA
		tmp2 = get_reg_val(sim->r, REG_RSP); //get %esp
		tmp2 -= 8; // subl $4, %esp
		set_reg_val(sim->r, REG_RSP, tmp2); //set %esp
		if(!set_long_val(sim->m, tmp2, tmp)) //set [%esp]
		{
			err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, tmp2);
			return STAT_ADR;
		}
		sim->pc = next_pc;
		break;
      case I_POPQ: /* B:0 regA:F */
        ...
        break;
      default:
    	err_print("PC = 0x%lx, Invalid instruction %.2x", sim->pc, codefun);  //%lx -> x
    	return STAT_INS;
    }
    return STAT_AOK;
}
